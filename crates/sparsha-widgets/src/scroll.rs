...
use sparsha_input::{InputEvent, Modifiers};
...
impl Scroll {
    ...
    fn content_size_from_tree(&self, layout_tree: &sparsha_layout::LayoutTree) -> (f32, f32) {
        if let Some(content) = &self.content {
            if let Some(content_layout) = layout_tree.get_absolute_layout(content.id()) {
                ...
                fn visit(
                    widget: &dyn Widget,
                    layout_tree: &sparsha_layout::LayoutTree,
                    min_x: &mut f32,
                    min_y: &mut f32,
                    max_x: &mut f32,
                    max_y: &mut f32,
    ...
    fn update_content_metrics(&mut self, viewport: Rect, layout_tree: &sparsha_layout::LayoutTree) {
