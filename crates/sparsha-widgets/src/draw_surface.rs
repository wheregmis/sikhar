...
//!
//! `DrawSurface` is the opt-in bridge between Sparsha's normal retained widget tree and a
//! scene-style draw callback.
//!
//! On native targets the scene is rendered through the shared GPU renderer.
//! On web targets Sparsha can embed the scene inside a dedicated `<canvas>` while still allowing
//! the widget to paint normal DOM-backed overlays through the regular `paint()` path.

use crate::{PaintCommands, PaintContext, Widget};
use sparsha_core::{Color, Point, Rect};
use sparsha_layout::{taffy, WidgetId};
