...
impl ForEach {
    fn push_child_key(seen: &mut HashSet<usize>, child_keys: &mut Vec<usize>, key: usize) {
        assert!(
            seen.insert(key),
            "ForEach received duplicate child key {key}"
        );
        child_keys.push(key);
    }
        ...
        W: IntoWidget,
    {
        ...
        for item in items {
            Self::push_child_key(&mut seen_keys, &mut child_keys, key_fn(&item).into());
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "ForEach received duplicate child key 10")]
    fn for_each_rejects_duplicate_child_keys() {
        let _ = ForEach::new(
